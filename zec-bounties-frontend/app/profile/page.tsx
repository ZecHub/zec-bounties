...
export default function ProfilePage() {
  ...
  // Shared save call — used by both the profile card (bio) and the privacy
  // card (visibility toggles), since the backend persists them together.
  ...
  const hasPaymentAddress = Boolean(currentUser?.UA_address);
    ...
    <ProtectedRoute blockAdmin>
      <div className="min-h-screen bg-background">
        ...
        <main className="container max-w-2xl mx-auto py-10 px-4 space-y-6">
          ...
          {/* ── Quick-glance status row: everything necessary at a glance ───── */}
          <div className="flex flex-wrap items-center gap-2">
            <Badge
              variant={hasPaymentAddress ? "secondary" : "outline"}
              className={cn(
                "gap-1.5 font-normal",
                hasPaymentAddress
            ...
            >
              ...
              {hasPaymentAddress ? "Payment address set" : "No payment address"}
            </Badge>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription className="mt-1">
                Your unified address where bounty rewards are sent. Must start
                with <code className="font-mono text-xs">u1</code>.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-1.5">
              ...
              {uaSaveState === "success" && (
                <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20">
                  ...
                  <div>
                    <p className="text-sm font-medium">Address saved</p>
                    <p className="text-xs text-muted-foreground">
                      You can now receive bounty payments.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <CardDescription className="mt-1">
                          For development bounties only. Your shielded or
                          unified address on the Zcash testnet — must start with{" "}
                          <code className="font-mono text-xs">u</code> or{" "}
                          <code className="font-mono text-xs">z</code>. Most
